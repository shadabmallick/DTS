...
public class DtsGalleryActivity extends AppCompatActivity implements ImageClickListner, View.OnClickListener,
    ...
    public void onImageMoved(final int movedCount, final int totalCount) {
        DtsGalleryActivity.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
    ...
    private void configureAlarm() {
        ...
        Intent intent = new Intent(DtsGalleryActivity.this, Scheduler.class);
...
//                alarmRequestCode, intent, PendingIntent.FLAG_NO_CREATE) != null);
        //  if(!isWorking) {
        PendingIntent pendingIntent = PendingIntent.getBroadcast(DtsGalleryActivity.this, alarmRequestCode, intent, 0);
        AlarmManager am = (AlarmManager) DtsGalleryActivity.this.getSystemService(DtsGalleryActivity.this.ALARM_SERVICE);
    ...
    private ArrayList<ImageBean> getDtsGalleryData() {
        isSavedImage = true;
        ArrayList<ImageBean> sinceInstallList = new Utils().fetchFolderList(DtsGalleryActivity.this);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (Settings.canDrawOverlays(DtsGalleryActivity.this)) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        startForegroundService(new Intent(DtsGalleryActivity.this, DtsWidget.class));
                    } else {
                        startService(new Intent(DtsGalleryActivity.this, DtsWidget.class));
                    }
                }
            } else {
                startService(new Intent(DtsGalleryActivity.this, DtsWidget.class));
    ...
    private void addToolTipView() {
        ...
        view.findViewById(R.id.keep24).setOnClickListener(DtsGalleryActivity.this);
        view.findViewById(R.id.keep2w).setOnClickListener(DtsGalleryActivity.this);
        view.findViewById(R.id.keep1w).setOnClickListener(DtsGalleryActivity.this);
        view.findViewById(R.id.keep1m).setOnClickListener(DtsGalleryActivity.this);
            ...
            if (viewID == R.id.ll_delete) {
                if (viewID == R.id.ll_delete) {
                    if (selectedImages != null && selectedImages.size() > 0) {
                        ...
                        Toast.makeText(DtsGalleryActivity.this,
    ...
    private ArrayList<Uri> getSelectedImageUri() {
        ...
        for (int i = 0; i < imageArrayList.size(); i++) {
            if (imageArrayList.get(i).isChecked() && imageArrayList.get(i).getViewType() == CHILD) {
                Uri uri;
                try {
                    uri = FileProvider.getUriForFile(DtsGalleryActivity.this,
    ...
    public void showConfrimDeleteDialog(final ArrayList<PhotoDetailBean> selectedImages) {
        ...
        builder = new AlertDialog.Builder(DtsGalleryActivity.this);
